...
run_dev:
	...
	docker run --name tinyproxy --rm -v /var/run/docker.sock:/var/run/docker.sock -v $(CURDIR):/usr/src/app -w /usr/src/app -p 8100:42069 tiny_proxy:latest ./bin/tinyproxy_dev
